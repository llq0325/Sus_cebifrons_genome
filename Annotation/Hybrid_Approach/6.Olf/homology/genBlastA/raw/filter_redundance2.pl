...
open OUT2,">$Outdir/all.nr.tab" or die "$!";
foreach my $name(keys %Net){

my @Nr;
foreach my $id ( sort keys %{$Net{$name}} ){
	@{$Net{$name}{$id}} = sort {$a->[7]<=>$b->[7]} @{$Net{$name}{$id}};
	foreach my $hsp ( @{$Net{$name}{$id}} ){
		if ( @Nr<1 ){
			push @Nr,[@$hsp];
		}elsif ( $Nr[-1][6] ne $id ){
			push @Nr,[@$hsp];
...
#	open OUT,">>$Outdir/all.nr.tab" or die "$!";
