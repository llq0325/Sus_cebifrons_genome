...
while(<IN>){
    ...
    my ($s_name,$add_start);
    if($scaffold=~/^([^\_]+)\_(\d+)/){
        ($s_name,$add_start)=($1,$2);
    }elsif($scaffold=~/^([^\d]+)\_([\d]+)/){
        ($s_name,$add_start)=($1,$2);
    }

	for(my $i=0;$i<@exon;$i++){
		my ($n_s,$n_e)=($1+$add_start-1,$2+$add_start-1) if($exon[$i]=~/(\d+)\S(\d+)/);
		$exon[$i]="$n_s,$n_e";
	}
	print join("\t",$id,$s_name,$Len{$pid},$start,$end,$len2,join(";",@sup),join(";",@exon))."\n";
}
close IN;
