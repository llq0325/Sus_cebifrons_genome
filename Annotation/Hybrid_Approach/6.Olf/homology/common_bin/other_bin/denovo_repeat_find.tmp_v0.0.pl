...
my $muscle_path="/opt/blc/genome/biosoft/muscle-3.8.31/bin/muscle";
...
if(defined $repeatmodeler)
{
		...
		if(defined $Node){
			$Node= "-l $Node";
			`nohup perl /ifs2/BC_GAG/Bin/Assembly/common_script/super_worker.pl -splitn 1 -absway -workdir ./ -verb -resource vf=5G $rmd_shell\n`;
		}
		else { `nohup  perl /ifs2/BC_GAG/Bin/Assembly/common_script/super_worker.pl -splitn 1 -absway -workdir ./ -verb -resource vf=5G  $rmd_shell\n`;}
...
if(defined $ltr_finder)
{
	...
	close OUT;
                if(defined $Node){
			$Node= "-l $Node";
			`nohup perl /ifs2/BC_GAG/Bin/Assembly/common_script/super_worker.pl -splitn 1 -absway -workdir ./ -verb -resource vf=3G $ltr_shell\n`;					
		 }
		else { `nohup  perl /ifs2/BC_GAG/Bin/Assembly/common_script/super_worker.pl -splitn 1 -absway -workdir ./ -verb -resource vf=3G  $ltr_shell\n`;}
}
