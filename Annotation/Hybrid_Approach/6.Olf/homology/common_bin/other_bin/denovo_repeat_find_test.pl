...
=head1 example
	...
	perl denovo_repeat_find.pl -Piler -RepeatScout -LTR_FINDER -RepeatModeler -cpu -run ./test.fa
...
my ($piler,$repeatscout,$Run,$tRNA,$ltr_finder,$repeatmodeler,$all,$Cpu,$Resource,$cutf,$cuts,$config,$Verbose,$help);
GetOptions(
	"Piler"=>\$piler,
...
my $muscle_path="/share/raid1/genome/bin/muscle";
...
$tRNA ||= "$ltr_path/tRNAdb.2010-2/Athal-tRNAs.fa";
my $dir=`pwd`;
chomp $dir;
print "$dir";

##############  build_library.pl PATH   #####################################
...
if(defined $piler){
		...
		#`perl $common_bin/qsub-sge.pl  -maxjob $Cpu --resource $Resource -reqsub find_pals.sh` if($Run eq "qsub");		
		`sh find_pals.sh`;
		#`mv final.all.hit.gff $hit`;
		`mv ../final.all.hit.gff $hit`;
...
if(defined $repeatscout){
		...
		#print OUT "perl $common_bin/qsub-sge.pl --lines 1 --resource  vf=5G  -reqsub  build_lmer_table.sh \n";	
		print OUT "sh build_lmer_table.sh \n";
		#print OUT "perl $common_bin/qsub-sge.pl --lines 1 --resource  vf=5G  -reqsub  repeatscout_out.sh  \n";
		print OUT "sh repeatscout_out.sh  \n";	
		print OUT "cat $repscout | $repeatscout_path/filter-stage-1.prl 1>$filter 2>>$log\n";
		#print OUT "RepeatMasker -nolow -no_is -norna -engine wublast -parallel 1 -lib $filter $seq_file 1>>$log\n  mv $seq_name.RepeatMasker.out $out\n" if($Run eq "multi");
		#print OUT "$find_repeat -repeatmasker -cutf $cutf -cpu $Cpu -lib $filter $seq_name\n mv $seq_name.RepeatMasker.out $out\n" if($Run eq "qsub");
		print OUT "/nas/GAG_02/zhouheling/RepeatModeler_software/software/RepeatMasker/RepeatMasker  -engine  wublast -lib $filter $seq_name\n mv $seq_name.RepeatMasker.out $out\n";
...
if(defined $ltr_finder)
{
		...
		print OUT "cat $outdir/*ltr_finder.gff >$seq_name.ltr_finder.gff\;";
		close OUT;
		#`nohup qsub -cwd -l vf=3G $ltr_shell\n`;		
		#`sh $ltr_shell\n`;
}
