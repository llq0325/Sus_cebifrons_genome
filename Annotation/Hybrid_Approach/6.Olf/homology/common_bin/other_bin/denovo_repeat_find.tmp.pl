...
my ($piler,$repeatscout,$Queue,$Run,$Node,$tRNA,$ltr_finder,$repeatmodeler,$all,$Cpu,$Resource,$cutf,$cuts,$Verbose,$help,$Pro_code);
GetOptions(
	"Piler"=>\$piler,
...
my $getTE=parse_config($config_file,"getTE.pl");
my $super_worker=parse_config($config_file,"super_worker.pl");
...
my $super_qp_para;
$super_qp_para.="-q $Queue " if (defined $Queue);
$super_qp_para.="-P $Pro_code " if (defined $Pro_code);
...
if(defined $piler){
		...
		`perl $pipeline $cut`;
		`perl $multi_process  -cpu $Cpu  find_pals.sh` if($Run eq "multi");
		if(defined $Node){
			`perl $qsub_sge $QP_para -maxjob $Cpu --resource $Resource --node $Node -reqsub --convert no find_pals.sh` if($Run eq "qsub");
		}
		else{
		`perl $qsub_sge $QP_para -maxjob $Cpu --resource $Resource  -reqsub --convert no find_pals.sh` if($Run eq "qsub");}
...
if(defined $repeatscout){
		...
		if(defined $Node){
			print OUT "perl $qsub_sge $QP_para --lines 1 --resource  vf=3G --node $Node -reqsub  build_lmer_table.sh \n";
		}
		else{print OUT "perl $qsub_sge $QP_para --lines 1 --resource  vf=3G  -reqsub  build_lmer_table.sh \n";}	
		if(defined $Node){
			print OUT "perl $qsub_sge $QP_para --lines 1 --resource  vf=3G --node $Node  -reqsub  repeatscout_out.sh  \n";
		}
		else{ print OUT "perl $qsub_sge $QP_para --lines 1 --resource  vf=3G  -reqsub  repeatscout_out.sh  \n";}	
...
if(defined $repeatmodeler)
{
		...
		if(defined $Node){
			$Node= "-l $Node";
			`nohup perl $super_worker -node $Node -qopts="$super_qp_para" -splitn 1 -absway -workdir ./ -verb -resource vf=5G $rmd_shell\n`;
		}
		else { `nohup  perl $super_worker -qopts="$super_qp_para" -splitn 1 -absway -workdir ./ -verb -resource vf=5G  $rmd_shell\n`;}
...
if(defined $ltr_finder)
{
	...
	foreach my $subfa(@fa)
	{
		...
		print OUT "$getTE $gff $cut_file > $outdir/$subfa.LTR.fa\n";
	}
	...
	close OUT;
                if(defined $Node){
			$Node= "-l $Node";
			`nohup perl $super_worker -node $Node -qopts="$super_qp_para" -splitn 1 -absway -workdir ./ -verb -resource vf=3G $ltr_shell\n`;					
		 }
		else { `nohup  perl $super_worker -qopts="$super_qp_para" -splitn 1 -absway -workdir ./ -verb -resource vf=3G  $ltr_shell\n`;}
}
