...
There is often problems happened due to memory, either because the svg figure is 
two large, or the machine has two small memory. So an option "--memory" is provied.

=head1 Version

  Author: Fan Wei, fanw@genomics.org.cn
  Version: 2.0,  Date: 2007-9-24
  Note:
...
=head1 Usage
  ...
  --memory <str>    set the maximum memory to use by java, eg, 1G, 100M, 100K, defalt=1G
...
my ($Type,$Ppi,$Width,$Height,$Memory);
my ($Verbose,$Help);
GetOptions(
	"type:s"=>\$Type,
	"ppi:s"=>\$Ppi,
...
foreach my $svg_file (@files) {
	...
	my $rasterizer_cmd = "$java -Djava.awt.headless=true -mx$Memory -jar $Bin/batik-1.7/batik-rasterizer.jar -m $Type -dpi $Ppi $xxx_file";
